...
const dataRL = require("./../data/rl.json");
  ...
  if((message.member.roles.find("name", "Alive") || message.member.roles.find("name", "Host")) && dataRL.w == true) {
    ...
    if (args[0].length === 1 && args[0] in dataID) {
      ...
      /*} else if (reciU.id == dataWL[message.author.id]) { // LOVER CHANNEL
      ...
      } else if (message.author.id == dataID[dataRL.mayor] && dataAT.revealed === true) {
        message.channel.send(":warning: **You cannot whisper when revealed!**");
        
      } else if (reciU.id == dataID[dataRL.mayor] && dataAT.revealed === true) {
        ...
        if (dataRL.bm.length != 0) {
          for (var i=0; i < dataRL.bm.length; i++) {
            client.channels.get(dataPP[dataRL.bm[i]]).send(":eye_in_speech_bubble: **"+sendN+"** ➜ **"+reciN+"**: *"+text+"*");
          }
        }
